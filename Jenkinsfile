        ...
        withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {        
            stage('Deployment') {
                if (isUnix()) {//Para sistemas Unix el comando varía un poco el formato
                    rc = sh returnStatus: true, script: "${toolbelt}/sfdx force:auth:logout --targetusername ${SF_USERNAME} -p" //Hacemos logout para evitar un error
                    // Autorizamos la dev hub org
                    rc = sh returnStatus: true, script: "${toolbelt}/sfdx force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${SF_USERNAME} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SF_HOST}"
